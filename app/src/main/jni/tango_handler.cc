...
int frame_number = 0;
int color_frame_number = 0;
double time_stamp_array[4000];

std::mutex queue_mutex;
SynchronizedQueue<cv::Mat> mImageQueue(0);
SynchronizedQueue<cv::Mat> mColorImageQueue(0);
SynchronizedQueue<double> mTimestampQueue(0);
SynchronizedQueue<double> mColorTimestampQueue(0);

SynchronizedQueue<std::list<float> > mXYZijQueue(0);
SynchronizedQueue<double> mXYZijTimeQueue(0);

std::thread consumer_color;
std::thread consumer_mono;
std::thread consumer_time;
std::thread consumer_depth;
...
/**
   ...
   **/
        ...
        frame_number++;
         LOGI("Stored mono frame number: %d ", frame_number);
...
**/
        ...
        queue_mutex.unlock();
         ...
         sprintf(newFileName,"/sdcard/color_imgs/img_%02d.jpg",color_frame_number);
         //cv::Mat frame = colorFrameList.front();
         //LOGI("size is : %d",sz);
         //cv::Mat frame2;
         ...
         cv::imwrite(newFileName,m2,compression_params);
...
**/
        queue_mutex.lock();
        mColorTimestampQueue.operate("push",NULL,&timestamp,NULL,NULL);
        queue_mutex.unlock();
...
static void saveColorFromQueue(std::string colorFilename, SynchronizedQueue<cv::Mat> *colorImageQueue, int* run){

        int cj = 0;
        unsigned int colorSz = 0;
        ...
        for(;;){
            ...
            if((*run) != 0 || colorSz != 0 ){
                ...
                queue_mutex.lock();
                (*colorImageQueue).operate("size",NULL,NULL,&colorSz,NULL);
                queue_mutex.unlock();
                    ...
                    queue_mutex.lock();
                    (*colorImageQueue).operate("size",NULL,NULL,&colorSz,NULL);
                    std::list<cv::Mat> colorFrameList(colorSz);
                    (*colorImageQueue).operate("drain",NULL,NULL,NULL,&colorFrameList);
                    queue_mutex.unlock();

                    unsigned int colorFrameListSz = colorFrameList.size();
                    LOGI("Color frames %d:", colorFrameListSz);
                    for(int i = 0; i< colorFrameListSz; i++){
                    char newFileName[300];
                    sprintf(newFileName,colorFilename.c_str(),cj);
                    cv::Mat frame = colorFrameList.front();
                    //LOGI("size is : %d",sz);
                    //cv::Mat frame2;
                    //cv::cvtColor(frame, frame2, CV_YUV2BGR_NV21,3);
                    ...
                    compression_params.push_back(CV_IMWRITE_JPEG_QUALITY);
                    compression_params.push_back(50);
                    cv::imwrite(newFileName,frame,compression_params);
                    delete [] frame.data;
                    frame.data = NULL;
                    colorFrameList.pop_front();
...
static void saveMonoFromQueue(std::string filename,SynchronizedQueue<cv::Mat> *imageQueue,int* run){
        ...
        for(;;){
            ...
            queue_mutex.unlock();
             ...
             if(sz != 0){
                        ...
                        LOGI("Mono frames %d:", frameListSz);
...
//static void saveColorTimeFromQueue(SynchronizedQueue<double> *colorTimestampQueue,int* run){}

static void saveTimeFromQueue(SynchronizedQueue<double> *timestampQueue,SynchronizedQueue<double> *colorTimestampQueue,SynchronizedQueue<double> *depthTimestampQueue,int* run){

     unsigned int timestampSz = 0;
     char timeFilename [100];
     unsigned int colorTimestampSz = 0;
     ...
     if((*run) != 0 || timestampSz != 0 || colorTimestampSz != 0 || depthTimeSz != 0 ){



         queue_mutex.lock();
         (*colorTimestampQueue).operate("size",NULL,NULL,&colorTimestampSz,NULL);
         queue_mutex.unlock();
        ...
        if(colorTimestampSz != 0){



            queue_mutex.lock();
            (*colorTimestampQueue).operate("size",NULL,NULL,&colorTimestampSz,NULL);
            std::list<double> colorTimeList(colorTimestampSz);
            (*colorTimestampQueue).operate("drain",NULL,NULL,NULL,&colorTimeList);
            queue_mutex.unlock();

            colorTimestampSz = colorTimeList.size();


                for(int i = 0; i< colorTimestampSz; i++){

                std::string text;
                sprintf((char*)text.c_str(),"%f,%f,%f,%f,%f\n",colorTimeList.front(),0.0,0.0,0.0,5.0);
                fp2.write(text.c_str(), strlen(text.c_str()));
                //LOGI("CRASHED AFTER POPPING TIMESTAMPS");
                colorTimeList.pop_front();
...
static void saveDepthFromQueue(SynchronizedQueue<std::list<float>> *depthQueue,int* run){
...
//static void saveDepthTimeFromQueue(*depthQueue,SynchronizedQueue<double> *depthTimestampQueue,int* run) {}
...
TangoErrorType TangoHandler::ConnectCameraCallback() {
  ...
  ret = TangoService_connectOnFrameAvailable(id,nullptr, onFrameAvailable);
  if (ret != TANGO_SUCCESS) {
               LOGE(
...
run_thread = 1;
//consumer_color = std::thread(&saveColorFromQueue,"/sdcard/color_imgs/color_img_%02d.jpg",&mColorImageQueue,&run_thread);
consumer_mono =  std::thread(&saveMonoFromQueue,"/sdcard/imgs/img_%02d.jpg",&mImageQueue,&run_thread);
consumer_time = std::thread(&saveTimeFromQueue,&mTimestampQueue,&mColorTimestampQueue,&mXYZijTimeQueue,&run_thread);
consumer_depth = std::thread(&saveDepthFromQueue,&mXYZijQueue,&run_thread );
